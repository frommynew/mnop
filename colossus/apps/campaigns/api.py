    ...
    if not is_test:
        ...
        if email.campaign.mailing_list.list_manager:
            list_subscribe_header.append(
                "<mailto:%s?subject=subscribe>"
                % email.campaign.mailing_list.list_manager
            )
            list_unsubscribe_header.append(
                "<mailto:%s?subject=unsubscribe>"
                % email.campaign.mailing_list.list_manager
            )
    ...
    with get_connection() as connection:
        for subscriber in campaign.get_recipients():
            if not subscriber.activities.filter(
                activity_type=ActivityTypes.SENT, email=campaign.email
            ).exists():
                sent = send_campaign_email_subscriber(
