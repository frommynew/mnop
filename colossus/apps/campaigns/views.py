...
class CampaignRevertDraftView(View):
    def post(self, request, pk):
        ...
        messages.success(
            request,
            gettext(
                "Campaign reverted to Draft status so you can make changes. Don't forget "
...
class CampaignReportsView(CampaignMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        unsubscribed_count = Activity.objects.filter(
            campaign_id=self.kwargs.get("pk"), activity_type=ActivityTypes.UNSUBSCRIBED
        ).count()

        subscriber_open_activities = (
            Activity.objects.filter(
                email__campaign_id=self.kwargs.get("pk"),
...
def load_list_tags(request):
    ...
    options = render_to_string(
        "campaigns/_campaign_list_tags_options.html", context, request
    )
    ...
    def get_initial(self):
        if self.campaign.mailing_list is not None:
            if self.campaign.email.from_email == "":
                self.initial[
                    "from_name"
                ] = self.campaign.mailing_list.campaign_default_from_name
                self.initial[
                    "from_email"
                ] = self.campaign.mailing_list.campaign_default_from_email
    ...
    def get_initial(self):
        if self.campaign.mailing_list is not None:
            if self.campaign.email.subject == "":
                self.initial[
                    "subject"
                ] = self.campaign.mailing_list.campaign_default_email_subject
        return super().get_initial()
...
class ScheduleCampaignView(CampaignMixin, UpdateView):
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .filter(status__in={CampaignStatus.DRAFT, CampaignStatus.SCHEDULED})
        )
...
def campaign_edit_content(request, pk):
    ...
    return render(
        request, "campaigns/email_form.html", {"campaign": campaign, "form": form}
    )
...
def campaign_test_email(request, pk):
    ...
    return render(
        request,
        "campaigns/test_email_form.html",
        {"menu": "campaigns", "campaign": campaign, "form": form},
    )
...
def campaign_preview_email(request, pk):
    ...
    if campaign.mailing_list:
        context["unsub"] = reverse(
            "subscribers:unsubscribe_manual",
            kwargs={"mailing_list_uuid": campaign.mailing_list.uuid},
        )
...
def send_campaign(request, pk):
    ...
    return render(
        ...
        {"menu": "campaign", "campaign": campaign},
    )
