...
class TestEmailEnableClickTracking(TestCase):
    ...
    def test_link_finder_regex_valid_links(self):
        ...
        valid_tags = [
            (
                "http://website1.com",
                "<a href=%s>Website 1</a>",
            ),  # no " or '
            (
                "http://website2.com",
                '<a href="%s">Website 2</a>',
            ),  # using "
            (
                "http://website3.com",
                "<a href='%s'>Website 3</a>",
            ),  # using '
            (
                "HTTP://WEBSITE4.COM",
                '<a href="%s">WEBSITE 4</a>',
            ),  # all caps
            (
                "http://website5.com",
                '<A HREF="%s">Website 5</A>',
            ),  # HREF caps
            (
                "https://website6.com",
                '<a href="%s">Website 6</a>',
            ),  # https
            (
                "https://website7.co.uk",
                '<a href="%s">Website 7</a>',
            ),  # other domain
            (
                "https://website8.co.uk:8080",
                '<a href="%s">Website 8</a>',
            ),  # using port
            (
                "http://website9.com/some/path/to/some/file.pdf",
                '<a href="%s">Website 8</a>',
            ),  # long path
            (
                "HTTPS://WEBSITE10.COM",
                '<a href="%s">WEBSITE 10</a>',
            ),  # all caps https
            ("http://127.0.0.1", '<a href="%s">IP Address</a>'),  # IP Address
            (
                "http://0.0.0.0:4200",
                '<a href="%s">IP Address with port</a>',
            ),  # IP Address with port
            (
                "https://www.amazon.com/gp/product/198302998X/ref=as_li_tl?ie=UTF8&amp;camp=1789&amp;creative=9325&amp;creativeASIN=198302998X&amp;linkCode=as2&amp;tag=vitorfs0b-20&amp;linkId=89931be04c94a3fd8c785b96746dd224",
                '<a href="%s">REST APIs with Django: Build powerful web APIs with Python and Django</a>',
            ),  # noqa
            (
                "https://simpleisbetterthancomplex.com",
                '<a href="%s">https://simpleisbetterthancomplex.com</a>',
            ),
        ]
    ...
    def test_link_finder_regex_invalid_links(self):
        ...
        invalid_tags = [
            (
                "{{unsub}}",
                '<a href="%s">Unsubscribe from this list</a>',
            ),
    ...
    def test_enable_click_tracking(self):
        ...
        self.assertEqual(
            4, models.Link.objects.filter(url="http://website.com").count()
        )
        self.assertEqual(
            2,
        ...
        self.assertEqual(
            2, models.Link.objects.filter(url="https://google.com").count()
        )
...
class TestEnableOpenTracking(TestCase):
    def test_valid_html_structure(self):
        email = factories.EmailFactory(
            template_content=("<!doctype html>" "<html>" "<body>" "</body>" "</html>")
        )
