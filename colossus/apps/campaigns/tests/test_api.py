...
class GetTestEmailContextTests(TestCase):
    ...
    def test_context_data_partial_override(self):
        ...
        expected = {"sub": "1", "unsub": "2", "name": "3", "uuid": "[SUBSCRIBER_UUID]"}
        self.assertDictEqual(actual, expected)
...
class SendCampaignTests(TestCase):
    def setUp(self):
        ...
        self.campaign = CampaignFactory(
            mailing_list=self.mailing_list, track_clicks=True, track_opens=True
        )
    ...
    def test_activity_sent_created(self):
        activities_count = Activity.objects.filter(
            activity_type=ActivityTypes.SENT
        ).count()
        self.assertEqual(activities_count, 10)

    def test_emails_sent(self):
        self.assertEqual(
            len(mail.outbox), 10, "Campaign must send 1 email for each subscriber."
        )
...
class SendCampaignEmailTestMailingListTests(TestCase):
    ...
    def test_email_contents_unsub_link(self):
        ...
        self.assertIn(
            unsubscribe_url, html_body, "Email HTML body must contain unsubscribe link."
        )
