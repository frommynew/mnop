...
class CampaignCreateViewTests(AuthenticatedTestCase):
    ...
    def test_initial_data(self):
        url = "{0}?mailing_list={1}&tag={2}".format(
            self.url, self.mailing_list_1.pk, self.tag_1.pk
        )
        response = self.client.get(url)
    ...
    def test_html(self):
        ...
        self.assertContains(
            response, '<input type="hidden"', 3
        )  # csrf_token, mailing_list, tag
...
class TestCampaignEditRecipientsViewNoneMailingList(AuthenticatedTestCase):
    def setUp(self):
        ...
        self.url = reverse(
            "campaigns:campaign_edit_recipients", kwargs={"pk": self.campaign.pk}
        )
        self.response = self.client.get(self.url)
