...
class Campaign(models.Model):
    ...
    campaign_type = models.PositiveSmallIntegerField(
        _("type"), choices=CampaignTypes.CHOICES, default=CampaignTypes.REGULAR
    )
    ...
    send_date = models.DateTimeField(
        _("send date"), null=True, blank=True, db_index=True
    )
    ...
    unique_opens_count = models.PositiveIntegerField(
        _("unique opens"), default=0, editable=False
    )
    total_opens_count = models.PositiveIntegerField(
        _("total opens"), default=0, editable=False
    )
    unique_clicks_count = models.PositiveIntegerField(
        _("unique clicks"), default=0, editable=False
    )
    total_clicks_count = models.PositiveIntegerField(
        _("total clicks"), default=0, editable=False
    )
    ...
    def email(self) -> Optional["Email"]:
        if not self.__cached_email and self.campaign_type == CampaignTypes.REGULAR:
            try:
                self.__cached_email, created = Email.objects.get_or_create(
                    campaign=self
                )
    ...
    def update_clicks_count_and_rate(self) -> tuple:
        ...
        qs = (
            Activity.objects.filter(
                email__campaign=self, activity_type=ActivityTypes.CLICKED
            )
        ...
        try:
            ...
            self.click_rate = 0.0
        self.save(
            update_fields=["unique_clicks_count", "total_clicks_count", "click_rate"]
        )
    ...
    def update_opens_count_and_rate(self) -> tuple:
        ...
        qs = (
            Activity.objects.filter(
                email__campaign=self, activity_type=ActivityTypes.OPENED
            )
            .values("subscriber_id")
            .order_by("subscriber_id")
            .aggregate(
        ...
        try:
            ...
            self.open_rate = 0.0
        self.save(
            update_fields=["unique_opens_count", "total_opens_count", "open_rate"]
        )
    ...
    def get_links(self) -> QuerySet:
        ...
        """
        links = Link.objects.filter(email__campaign=self).order_by(
            "-total_clicks_count"
        )
        return links
...
class Email(models.Model):
    ...
    def checklist(self) -> dict:
        ...
        if (
            self.campaign.mailing_list is not None
            and self.campaign.mailing_list.get_active_subscribers().exists()
        ):
    ...
    def build_child_template_string(self) -> str:
        ...
        """
        virtual_template = [
            "{%% extends %s %%}" % self.BASE_TEMPLATE_VAR,
        ]
        blocks = self.get_blocks()
        for block_key, block_content in blocks.items():
            if block_content:
                virtual_template.append(
                    "{%% block %s %%}\n%s\n{%% endblock %%}"
                    % (block_key, block_content)
                )
    ...
    def _enable_click_tracking(self, html, index=0):
        ...
        for data in urls:
            ...
            track_url = "%s://%s/track/click/%s/{{uuid}}/" % (
                protocol,
                domain,
                link.uuid,
            )
    ...
    def enable_click_tracking(self):
        self.template_content, index = self._enable_click_tracking(
    ...
    def update_opens_count(self) -> tuple:
        qs = (
            self.activities.filter(activity_type=ActivityTypes.OPENED)
            .values("subscriber_id")
            .order_by("subscriber_id")
            .aggregate(
                unique_count=Count("subscriber_id", distinct=True),
...
class Link(models.Model):
    ...
    unique_clicks_count = models.PositiveIntegerField(
        _("unique clicks count"), default=0, editable=False
    )
    total_clicks_count = models.PositiveIntegerField(
        _("total clicks count"), default=0, editable=False
    )
    ...
    def update_clicks_count(self) -> tuple:
        ...
        """
        qs = (
            self.activities.values("subscriber_id")
            .order_by("subscriber_id")
            .aggregate(
                unique_count=Count("subscriber_id", distinct=True),
                total_count=Count("subscriber_id"),
