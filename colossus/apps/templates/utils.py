...
def wrap_blocks(template_source):
    ...
    for block_name in block_names:
        template_source = re.sub(
            NAMED_BLOCK_RE % block_name, START_BLOCK_DIV % block_name, template_source
        )
...
def get_template_variables(template):
    ...
    """
    clean_variables = filter(
        lambda _v: not _v.startswith("block"), _get_template_variables(template)
    )
