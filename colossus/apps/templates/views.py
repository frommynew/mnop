...
class EmailTemplateListView(EmailTemplateMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("q", ""):
            ...
            self.extra_context.update({"is_filtered": True, "query": query})
        queryset = queryset.order_by(
            F("last_used_date").desc(nulls_last=True), "-update_date"
        )
        return queryset
...
def email_template_editor(request, pk):
    ...
    return render(
        request,
        "templates/emailtemplate_editor.html",
        {"menu": "templates", "form": form, "email_template": email_template},
    )
