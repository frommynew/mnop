...
class TrackOpenTests(TestCase):
    ...
    def test_subscriber_opened_email(self):
        self.assertTrue(
            Activity.objects.filter(activity_type=ActivityTypes.OPENED).exists()
        )
...
class TrackClickTests(TestCase):
    ...
    def test_redirection(self):
        self.assertRedirects(
            self.response, self.link.url, fetch_redirect_response=False
        )

    def test_subscriber_clicked_link(self):
        self.assertTrue(
            Activity.objects.filter(activity_type=ActivityTypes.CLICKED).exists()
        )
...
class TestPostUnsubscribeManualSuccessful(TestCase):
    def setUp(self):
        ...
        self.url = reverse(
            ...
            kwargs={"mailing_list_uuid": self.subscriber.mailing_list.uuid},
        )
        self.response = self.client.post(
            self.url, data={"email": self.subscriber.email}
        )
...
class TestGetUnsubscribeManual(TestCase):
    def setUp(self):
        ...
        url = reverse(
            "subscribers:unsubscribe_manual",
            kwargs={"mailing_list_uuid": mailing_list.uuid},
        )
