...
class SubscriberOpenEmailTests(TestCase):
    def setUp(self):
        ...
        self.subscriber_1.create_activity(
            ActivityTypes.SENT, email=self.email
        )  # mock email sent activity
...
class SubscriberClickLinkTests(TestCase):
    def setUp(self):
        ...
        self.subscriber_1 = SubscriberFactory(mailing_list=mailing_list)
        self.subscriber_1.create_activity(
            ActivityTypes.SENT, email=self.link.email
        )  # mock email sent activity
        self.subscriber_2 = SubscriberFactory(mailing_list=mailing_list)
        self.subscriber_2.create_activity(
            ActivityTypes.SENT, email=self.link.email
    ...
    def test_click_link_once(self):
        ...
        self.assertEqual(
            1,
            self.subscriber_1.activities.filter(
                activity_type=ActivityTypes.CLICKED
            ).count(),
        )
...
class SubscriberClickLinkForceOpenTests(TestCase):
    ...
    def test_click_twice_without_open(self):
        ...
        self.subscriber.click(
            self.link
        )  # this time it should not trigger the `open` method
...
class SubscriberUpdateClickRateTests(TestCase):
    ...
    def test_click_rate_persistence(self):
        ...
        self.subscriber.create_activity(ActivityTypes.SENT, email=self.email)
        self.subscriber.create_activity(
            ActivityTypes.CLICKED, email=self.email, link=self.link
        )
...
class SubscriptionFormTemplateTests(TestCase):
    def setUp(self):
        ...
        self.form_template = SubscriptionFormTemplateFactory(
            key=TemplateKeys.SUBSCRIBE_FORM, mailing_list=mailing_list
        )
    ...
    def test_get_default_content(self):
        ...
        self.assertHTMLEqual(
            html, '<h5 class="card-title">Subscribe to our mailing list</h5>'
        )
    ...
    def test_get_default_subject(self):
        ...
        self.assertEqual(
            self.form_template.get_default_subject(), "Subscription Confirmed"
        )
...
class SubscriptionFormTemplateLoadDefaultsTests(TestCase):
    ...
    def test_load_defaults(self):
        ...
        self.assertHTMLEqual(
            form_template.content_html,
            '<h5 class="card-title">Subscribe to our mailing list</h5>',
        )
