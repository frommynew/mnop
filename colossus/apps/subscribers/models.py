...
from colossus.apps.subscribers.exceptions import (
    FormTemplateIsNotEmail,
    FormTemplateIsNotForm,
)
...
class Tag(models.Model):
    ...
    mailing_list = models.ForeignKey(
        MailingList, on_delete=models.CASCADE, related_name="tags"
    )
    ...
    def get_absolute_url(self) -> str:
        return reverse(
            "lists:edit_tag", kwargs={"pk": self.mailing_list_id, "tag_pk": self.pk}
        )
...
class Subscriber(models.Model):
    ...
    mailing_list = models.ForeignKey(
        MailingList, on_delete=models.PROTECT, related_name="subscribers"
    )
    ...
    status = models.PositiveSmallIntegerField(
        _("status"), default=Status.PENDING, choices=Status.CHOICES
    )
    optin_ip_address = models.GenericIPAddressField(
        _("opt-in IP address"), unpack_ipv4=True, blank=True, null=True
    )
    optin_date = models.DateTimeField(_("opt-in date"), default=timezone.now)
    confirm_ip_address = models.GenericIPAddressField(
        _("confirm IP address"), unpack_ipv4=True, blank=True, null=True
    )
    confirm_date = models.DateTimeField(_("confirm date"), null=True, blank=True)
    last_seen_ip_address = models.GenericIPAddressField(
        _("last seen IP address"), unpack_ipv4=True, blank=True, null=True
    )
    ...
    last_sent = models.DateTimeField(
        _("last campaign sent date"), null=True, blank=True
    )
    tags = models.ManyToManyField(
        Tag, related_name="subscribers", verbose_name=_("tags"), blank=True
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "lists:edit_subscriber",
            kwargs={"pk": self.mailing_list_id, "subscriber_pk": self.pk},
        )
    ...
    def delete(self, using=None, keep_parents=False):
        email_ids = list(
            self.activities.filter(activity_type=ActivityTypes.SENT).values_list(
                "email_id", flat=True
            )
        )
        link_ids = list(
            self.activities.filter(activity_type=ActivityTypes.CLICKED)
            .values_list("link_id", flat=True)
            .order_by("link_id")
            .distinct()
        )
        super().delete(using, keep_parents)
        update_rates_after_subscriber_deletion.delay(
            self.mailing_list_id, email_ids, link_ids
        )
    ...
    def unsubscribe(self, request, campaign=None):
        ...
        with transaction.atomic():
            ...
            self.create_activity(
                ActivityTypes.UNSUBSCRIBED, campaign=campaign, ip_address=ip_address
            )
    ...
    def create_activity(self, activity_type, **activity_kwargs):
        activity_kwargs.update({"subscriber": self, "activity_type": activity_type})
        activity = Activity.objects.create(**activity_kwargs)
        return activity

    def get_activities(self, **filter_kwargs):
        return (
            self.activities.select_related(
                "subscriber__mailing_list", "email__campaign", "link"
            )
    ...
    def open(self, email, ip_address=None):
        ...
        if ip_address is not None:
            self.create_activity(
                ActivityTypes.OPENED, email=email, ip_address=ip_address
            )
            update_subscriber_location.delay(ip_address, self.pk)
        else:
            self.create_activity(
                ActivityTypes.OPENED, email=email, location=self.location
            )
        update_open_rate.delay(self.pk, email.pk)

    def click(self, link, ip_address=None):
        self.create_activity(
            ActivityTypes.CLICKED, link=link, email=link.email, ip_address=ip_address
        )
        if ip_address is not None:
            ...
            self.activities.filter(
                activity_type=ActivityTypes.OPENED, email=link.email, ip_address=None
    ...
    def update_open_rate(self) -> float:
        count = self.activities.values("email_id", "activity_type").aggregate(
            sent=Count(
                "email_id", distinct=True, filter=Q(activity_type=ActivityTypes.SENT)
            ),
            opened=Count(
                "email_id", distinct=True, filter=Q(activity_type=ActivityTypes.OPENED)
            ),
        )
        try:
    ...
    def update_click_rate(self) -> float:
        count = self.activities.values("email_id", "activity_type").aggregate(
            ...
            clicked=Count(
                "email_id", distinct=True, filter=Q(activity_type=ActivityTypes.CLICKED)
            ),
        )
        try:
...
class Activity(models.Model):
    activity_type = models.PositiveSmallIntegerField(
        _("type"), choices=ActivityTypes.CHOICES
    )
    ...
    subscriber = models.ForeignKey(
        Subscriber, on_delete=models.CASCADE, related_name="activities"
    )
    ...
    link = models.ForeignKey(
        Link, on_delete=models.CASCADE, null=True, blank=True, related_name="activities"
    )
...
class SubscriptionFormTemplate(models.Model):
    key = models.CharField(
        _("key"), choices=TemplateKeys.CHOICES, max_length=30, db_index=True
    )
    mailing_list = models.ForeignKey(
        MailingList,
        on_delete=models.CASCADE,
    ...
    def get_default_subject(self):
        ...
        subject = render_to_string(
            subject_template_name, {"list_name": self.mailing_list.name}
        )
