...
def update_open_rate(subscriber_id, email_id):
    ...
    try:
        subscriber = (
            Subscriber.objects.filter(pk=subscriber_id)
        ...
        logger.exception(
            "An error occurred while trying to update open rates with "
...
def update_click_rate(subscriber_id, link_id):
    ...
    try:
        ...
        with transaction.atomic():
            if not subscriber.activities.filter(
                activity_type=ActivityTypes.OPENED, email=link.email
            ).exists():
                ...
                activity = subscriber.activities.filter(
                    activity_type=ActivityTypes.CLICKED, link=link
                ).first()
...
def update_rates_after_subscriber_deletion(mailing_list_id, email_ids, link_ids):
    ...
    Email = apps.get_model("campaigns", "Email")
    emails = (
        Email.objects.filter(pk__in=email_ids)
        .select_related("campaign")
        .only("pk", "campaign__id")
    )
    for email in emails:
...
def update_subscriber_location(ip_address, subscriber_id):
    ...
    if location is not None:
        ...
        if (
            subscriber.last_seen_ip_address == ip_address
            and subscriber.location_id != location.pk
        ):
            subscriber.location = location
            subscriber.save(update_fields=["location"])

        subscriber.activities.filter(ip_address=ip_address).filter(
            Q(location=None) | Q(activity_type=ActivityTypes.OPENED)
