...
class SubscribeForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if is_subscribed:
            email_validation_error = ValidationError(
                gettext(
                    'The email address "%(email)s" is already subscribed to this list.'
                ),
                params={"email": email},
    ...
    def subscribe(self, request):
        ...
        path = reverse(
            "subscribers:confirm_double_optin_token",
            kwargs={"mailing_list_uuid": self.mailing_list.uuid, "token": token.text},
        )
        confirm_link = "%s://%s%s" % (protocol, domain, path)

        confirm_email = self.mailing_list.get_confirm_email_template()
        confirm_email.send(subscriber.get_email(), {"confirm_link": confirm_link})
...
class UnsubscribeForm(forms.Form):
    ...
    def clean(self):
        ...
        if not is_subscribed:
            email_validation_error = ValidationError(
                gettext(
                    'The email address "%(email)s" is not subscribed to this list.'
                ),
                params={"email": email},
