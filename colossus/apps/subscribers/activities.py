...
UNSUBSCRIBED_TEMPLATE = (
    '<small class="text-muted">%s</small> <strong>Unsubscribed</strong>.'
)
...
IMPORTED_TEMPLATE = (
    '<small class="text-muted">%s</small> <strong>Imported</strong> to the List %s'
)

CLEANED_TEMPLATE = (
    '<small class="text-muted">%s</small> <strong>Cleaned</strong> from the List.'
)


def render_unsubscribe_activity(activity):
    if activity.campaign is not None:
        return UNSUBSCRIBED_CAMPAIGN_TEMPLATE % (
            activity.get_formatted_date(),
...
def render_activity(activity):
    ...
    renderers = {
        ...
        % (a.subscriber.mailing_list.name, a.get_formatted_date()),
