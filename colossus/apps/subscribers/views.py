...
def subscribe(request, mailing_list_uuid):
    ...
    if is_limited:
        ...
        messages.warning(
            request, _("Too many requests. Your IP address is blocked for 5 minutes.")
        )
    ...
    if request.method == "POST" and not is_limited:
        ...
        if valid_recaptcha_or_skip_recaptcha_validation:
            if form.is_valid():
                ...
                return redirect(
                    "subscribers:confirm_subscription",
                    mailing_list_uuid=mailing_list_uuid,
                )
        else:
            messages.info(
                request, _("Please check the reCAPTCHA and submit the form again.")
            )
    else:
    ...
    return render(
        request,
        "subscribers/subscribe_form.html",
        {"mailing_list": mailing_list, "form": form, "content": content},
    )
...
def confirm_subscription(request, mailing_list_uuid):
    ...
    return render(
        request,
        "subscribers/subscribe_thank_you.html",
        {"mailing_list": mailing_list, "content": form_template.content_html},
    )
...
def confirm_double_optin_token(request, mailing_list_uuid, token):
    try:
        ...
        return HttpResponseBadRequest(
            "The requested list does not exist.", content_type="text/plain"
        )

    try:
        confirm_token = Token.objects.get(
            text=token, description="confirm_subscription"
        )
    ...
    return render(
        request,
        "subscribers/confirm_thank_you.html",
        {"mailing_list": mailing_list, "content": form_template.content_html},
    )
...
def unsubscribe_manual(request, mailing_list_uuid):
    ...
    return render(
        request,
        "subscribers/unsubscribe_form.html",
        {"form": form, "mailing_list": mailing_list, "content": content},
    )
...
def unsubscribe(request, mailing_list_uuid, subscriber_uuid, campaign_uuid):
    ...
    try:
        subscriber = Subscriber.objects.get(
            uuid=subscriber_uuid, mailing_list=mailing_list
        )
    except Subscriber.DoesNotExist:
        return redirect(
            "subscribers:unsubscribe_manual", mailing_list_uuid=mailing_list_uuid
        )
    ...
    if subscriber.status != Status.SUBSCRIBED:
        return HttpResponse(
            "This email address was not found in our list.", content_type="text/plain"
        )
...
def goodbye(request, mailing_list_uuid):
    ...
    return render(
        ...
        {"mailing_list": mailing_list, "content": content},
    )
...
def track_open(request, email_uuid, subscriber_uuid):
    try:
        ...
        logger.exception(
            'An error occurred while subscriber "%s" was trying to '
...
@ratelimit(key=ip_address_key, rate="100/h", method="GET", block=True)
def track_click(
    request: HttpRequest, link_uuid: UUID, subscriber_uuid: UUID
) -> HttpResponseRedirect:
    """
    ...
    try:
        ...
        # fail silently
        logger.info(
            "track_click call to non-existing Subscriber instance"
            'uuid = "%s"' % str(subscriber_uuid)
        )
    except Exception:
        logger.exception(
            'Failed to track click on link "%s" from subscriber '
