...
def clean_list_task(mailing_list_id):
    try:
        ...
        data = {"mailing_list_id": mailing_list_id, "cleaned": 0}
        ...
        if settings.MAILGUN_API_KEY:
            ...
            if "items" in bounces:
                for bounce in bounces["items"]:
                    ...
                    if (
                        mailing_list.get_active_subscribers()
        ...
        if data["cleaned"] > 0:
            ...
            for user in User.objects.filter(is_superuser=True, is_active=True):
                Notification.objects.create(
                    user=user, action=Actions.LIST_CLEANED, text=text
                )
    ...
    try:
        ...
        if subscriber_import.status == ImportStatus.QUEUED:
            ...
            try:
                ...
                with open(subscriber_import.file.path, "r") as csvfile:
                    ...
                    with transaction.atomic():
                        for row in reader:
                            ...
                            ) in columns_mapping.items():
                                field_parser = ImportFields.PARSERS[
                                    subscriber_field_name
                                ]
                            ...
                            try:
                                ...
                                email_domain, created = Domain.objects.get_or_create(
                                    name=domain_name
                                )
                            ...
                            if subscriber_import.strategy == ImportStrategies.CREATE:
                                ...
                                if subscriber_exists:
                                    subscriber_queryset.update(
                                        update_date=timezone.now(), **defaults
                                    )
                ...
                output_message = (
                    'The subscriber import "%s" completed with success. %s created, %s updated, '
                    "%s skipped."
                    % (
                ...
                output_message = (
                    'An error occurred while importing the file "%s".'
                    % subscriber_import_id
                )
            ...
            return (
                'The subscriber import file "%s" was not queued to be imported.'
                % subscriber_import_id
            )
