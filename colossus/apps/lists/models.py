...
class MailingList(models.Model):
    ...
    website_url = models.URLField(
        _("website URL"),
        blank=True,
        help_text=_("Where did people opt in to this list?"),
    )
    campaign_default_from_name = models.CharField(
        _("default from name"), max_length=100, blank=True
    )
    campaign_default_from_email = models.EmailField(
        _("default from email address"), blank=True
    )
    campaign_default_email_subject = models.CharField(
        _("default subject"), max_length=150, blank=True
    )
    ...
    list_manager = models.EmailField(
        ...
        blank=True,
        help_text=_(
            "Email address to handle subscribe/unsubscribe requests."
    ...
    forms_custom_header = models.TextField(
        _("custom header"),
        help_text=_(
            "Header displayed on all subscription form pages. Accepts HTML."
    ...
    def update_open_and_click_rate(self):
        qs = (
            self.get_active_subscribers()
            .exclude(last_sent=None)
            .aggregate(open=Avg("open_rate"), click=Avg("click_rate"))
        )
    ...
    def _get_form_template(self, form_template_key: str):
        form_template, created = self.forms_templates.get_or_create(
...
class SubscriberImport(models.Model):
    ...
    file = models.FileField(
        _("CSV file"), upload_to="uploads", storage=PrivateMediaStorage()
    )
    ...
    status = models.PositiveSmallIntegerField(
        _("status"), default=ImportStatus.PENDING, choices=ImportStatus.CHOICES
    )
    ...
    strategy = models.PositiveSmallIntegerField(
        ...
        help_text=_(
            "The email address will be used as the main subscriber identifier to determine if they are "
            "already on the list."
        ),
    )
