...
class ConfirmSubscriberImportForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        for index, heading in enumerate(self.headings):
            self.fields[self._field_key(index)] = forms.ChoiceField(
                label=heading, required=False, choices=choices
            )
            try:
    ...
    def column_mapping_fields(self) -> List[BoundField]:
        ...
        return [
            ...
            if field.name.startswith("__column_")
        ]
    ...
    def import_settings_fields(self) -> List[BoundField]:
        ...
        return [
            field
            for field in self.visible_fields()
            if not field.name.startswith("__column_")
        ]
...
class PasteImportSubscribersForm(forms.Form):
    emails = MultipleEmailField(
        ...
        help_text=_(
            "One email per line, or separated by comma. Duplicate emails will be suppressed."
        ),
    )
        ...
        with transaction.atomic():
            for email in emails:
                ...
                subscriber, created = Subscriber.objects.get_or_create(
                    ...
                    defaults={"email": email, "domain": domain},
                )
                if created:
...
class BulkTagForm(forms.Form):
    ...
    emails = MultipleEmailField(
        label=_("Paste email addresses"),
        help_text=_(
            "One email per line, or separated by comma. "
        ...
        for email in emails:
            try:
                subscriber = Subscriber.objects.get(
                    mailing_list=self.mailing_list, email__iexact=email
                )
