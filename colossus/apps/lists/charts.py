    ...
    def get_data(self):
        ...
        activities = (
            Activity.objects.filter(
                subscriber__mailing_list=self.mailing_list,
                date__date__gt=thirty_days_ago,
            )
            .values("date__date")
            .annotate(
                subscribed=subscribed_expression, unsubscribed=unsubscribed_expression
            )
            .order_by("date__date")
        )
        ...
        for key, value in sorted(
            series.items(), key=lambda e: e[1]["order"], reverse=True
        ):
            labels.append(key)
...
class ListLocationsChart(DoughnutChart):
    def get_data(self):
        locations = (
            self.mailing_list.get_active_subscribers()
