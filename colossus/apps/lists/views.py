...
class MailingListCreateView(CreateView):
    ...
    fields = (
        "name",
        "slug",
        "campaign_default_from_name",
        "campaign_default_from_email",
        "website_url",
    )
...
class MailingListDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs) -> Dict:
        locations = (
            self.object.get_active_subscribers()
        ...
        subscribed_expression = Count(
            "id", filter=Q(activity_type=ActivityTypes.SUBSCRIBED)
        )
        unsubscribed_expression = Count(
            "id", filter=Q(activity_type=ActivityTypes.UNSUBSCRIBED)
        )
        cleaned_expression = Count("id", filter=Q(activity_type=ActivityTypes.CLEANED))
        summary_last_30_days = Activity.objects.filter(
            subscriber__mailing_list=self.object, date__gte=thirty_days_ago
        ).aggregate(
            subscribed=subscribed_expression,
        ...
        kwargs["subscribed_count"] = self.object.subscribers.filter(
            status=Status.SUBSCRIBED
        ).count()
        kwargs["unsubscribed_count"] = self.object.subscribers.filter(
            status=Status.UNSUBSCRIBED
        ).count()
        kwargs["cleaned_count"] = self.object.subscribers.filter(
...
class SubscriberListView(MailingListMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("q", ""):
            ...
            self.extra_context = {"is_filtered": True, "query": query}
...
class SubscriptionFormsView(MailingListMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["sub"] = get_absolute_url(
            "subscribers:subscribe", {"mailing_list_uuid": self.mailing_list.uuid}
        )
        kwargs["sub_short"] = get_absolute_url(
            "subscribe_shortcut", {"mailing_list_slug": self.mailing_list.slug}
        )
        ...
        kwargs["unsub_short"] = get_absolute_url(
            "unsubscribe_shortcut", {"mailing_list_slug": self.mailing_list.slug}
        )
        return super().get_context_data(**kwargs)
...
class TagCreateView(TagMixin, MailingListMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        tag.save()
        messages.success(
            self.request, _('Tag "%(name)s" created with success.') % form.cleaned_data
        )
...
class CampaignDefaultsView(AbstractSettingsView):
    fields = (
        "campaign_default_from_name",
        "campaign_default_from_email",
        "campaign_default_email_subject",
    )
...
@method_decorator(login_required, name="dispatch")
class SubscriptionFormTemplateUpdateView(
    FormTemplateMixin, MailingListMixin, UpdateView
):
...
class SubscriberImportView(MailingListMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        return redirect(
            "lists:import_preview", pk=mailing_list_id, import_pk=subscriber_import.pk
        )
...
class SubscriberImportPreviewView(MailingListMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse(
            "lists:csv_import_subscribers", kwargs={"pk": self.kwargs.get("pk")}
        )
...
class ChartView(View):
    ...
    def get(self, request, pk):
        try:
            ...
            return JsonResponse(
                data={"message": gettext("Invalid mailing list id.")}, status_code=400
            )
...
def download_subscriber_import(request, pk, import_pk):
    subscriber_import = get_object_or_404(
        SubscriberImport, pk=import_pk, mailing_list_id=pk
    )
