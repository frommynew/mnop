...
class TagListViewTests(TagTestCase):
    ...
    def test_html_content(self):
        ...
        for tag in self.tags:
            edit_url = reverse(
                "lists:delete_tag", kwargs={"pk": tag.mailing_list.pk, "tag_pk": tag.pk}
            )
            delete_url = reverse(
                "lists:delete_tag", kwargs={"pk": tag.mailing_list.pk, "tag_pk": tag.pk}
            )
...
class BulkTagSubscribersViewSuccessTests(TagTestCase):
    def setUp(self):
        ...
        self.subscriber = SubscriberFactory(
            mailing_list=self.mailing_list, email="john@example.com"
        )
        self.tag = self.tags[0]
        self.url = reverse("lists:bulk_tag", kwargs={"pk": self.mailing_list.pk})
        self.response = self.client.post(
            self.url, {"tag": self.tag.pk, "emails": self.subscriber.email}
        )
