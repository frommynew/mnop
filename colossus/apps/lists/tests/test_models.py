...
class MailingListUpdateSubscribersCountTests(TestCase):
    ...
    def test_update_subscribers_count_persistence(self):
        self.assertEqual(
            0, MailingList.objects.get(pk=self.mailing_list.pk).subscribers_count
        )
        SubscriberFactory(mailing_list=self.mailing_list, status=Status.SUBSCRIBED)
        self.mailing_list.update_subscribers_count()
        self.assertEqual(
            1, MailingList.objects.get(pk=self.mailing_list.pk).subscribers_count
        )
    ...
    def test_subscribed_count_5_subscribers(self):
        SubscriberFactory.create_batch(
            5, mailing_list=self.mailing_list, status=Status.SUBSCRIBED
        )
...
class MailingListUpdateOpenRateTests(TestCase):
    ...
    def test_persistence(self):
        ...
        self.assertEqual(
            0.5, MailingList.objects.get(pk=self.mailing_list.pk).open_rate
        )
...
class MailingListUpdateClickRateTests(TestCase):
    ...
    def test_persistence(self):
        ...
        self.assertEqual(
            0.5, MailingList.objects.get(pk=self.mailing_list.pk).click_rate
        )
