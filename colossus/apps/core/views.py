...
def dashboard(request):
    ...
    activities = (
        Activity.objects.select_related("campaign", "subscriber__mailing_list")
        .filter(
            activity_type__in={ActivityTypes.SUBSCRIBED, ActivityTypes.UNSUBSCRIBED}
        )
        .order_by("-date")[:50]
    )
    return render(
        request,
        "core/dashboard.html",
        {"menu": "dashboard", "activities": activities, "drafts": campaigns},
    )
...
def setup(request):
    ...
    if site.domain == "nem.nanokernel.com":
        site.name = "NEM"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                _(
                    "Account created with success! Go ahead and create your first mailing list."
                ),
            )
...
def unsubscribe_shortcut(request, mailing_list_slug):
    mailing_list = get_object_or_404(MailingList, slug=mailing_list_slug)
    return redirect(
        "subscribers:unsubscribe_manual", mailing_list_uuid=mailing_list.uuid
    )
